    ...
    foreach($groups as $group) {
        ...
        if( isset($scriptProperties['groups_'.$id]) ) {
            ...
            if ( $myGroup ) {
                // this already exists
                $myGroup->get('id');
                $modx->log(modX::LOG_LEVEL_ERROR,'[Group ELetters/Process/Update] Subscriber('.$subscriber->get('id') .') Exists for group: '.$id.' ID: '.$myGroup->get('id'));
                ...
                $modx->log(modX::LOG_LEVEL_ERROR,'[Group ELetters/Process/Update] ADD Subscriber('.$subscriber->get('id') .') to group: '.$id);
