...
/* get obj */
if (empty($_DATA['id'])) return $modx->error->failure($modx->lexicon('groupeletters.subscribers.err.nf'));
$subscriber = $modx->getObject('EletterSubscribers',$_DATA['id']);
if (empty($subscriber)) return $modx->error->failure($modx->lexicon('groupeletters.subscribers.err.nf'));
