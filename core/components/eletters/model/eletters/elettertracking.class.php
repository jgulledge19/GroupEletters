...
class EletterTracking {
    ...
    public function makeUrl ($url,$newsletterID, $type='click') {
        ...
        if ( $type == 'image' ){
            $url = $this->modx->getOption('site_url').$this->modx->getOption('assets_url').'components/eletters/?t='.base_convert($this->created_urls[$url],10,36).'|[[+code]]&amp;';
        } else {
            $url = $this->modx->makeUrl($this->modx->getOption('eletters.trackingPageID', '', 1),'', array('t' => base_convert($this->created_urls[$url],10,36).'|[[+code]]'), 'http');
        } 
    ...
    public function logAction($type='click') {
        ...
        if ( $type == 'image') {
            ...
            if ( isset($_GET['image']) ) {
                ...
                $filename = MODX_ASSETS_PATH.'components'.DIRECTORY_SEPARATOR.'eletters'.DIRECTORY_SEPARATOR.'images'.DIRECTORY_SEPARATOR.$filename;
                if ( !file_exists($filename) ) {
                    $filename = 'clear.gif';
                }
            } else {
                $filename = MODX_ASSETS_PATH.'components'.DIRECTORY_SEPARATOR.'eletters'.DIRECTORY_SEPARATOR.'images'.DIRECTORY_SEPARATOR.'clear.gif';
            }
