...
Features
    ...
    Basic Statistics - Set the URL Tracking (eletters.useUrlTracking) to Yes
        ...
        - Messages Opened. This is the number of recipients who open your email to read it. Due to the way open rates are 
            ...
            Use [[+trackingImage]] for an image, you can also put a custom image/banner doing this: [[+trackingImage]]image=test.jpg then put the image in assets/components/eletters/images/
...
Docs: http://rtfm.modx.com/display/ADDON/Eletters
==========================================
 Installation
==========================================
First you will need to install through Package Management and then do the following set up steps:
* Create a signup page look at the example in the EletterSignup Chunk
* Optional - Create a "Thank you" page (and set it as 'redirectTo' in your signup page FormIt snippet call)
* Create a confirm / opt-in page just add in the  [[!EletterConfirm]] Snippet and set this pages id in 
    the signup page FormIt snippet call
* Create a unsubscribe page, just add in the [[!EletterUnsubscribe]] Snippet.  Make sure you add a link 
    to this page in your newsletter template [[+unsubscribeUrl]]
* Go to System->System Setting and select Eletters and change the settings to match what you want and the 
    pages you just created.
* Go to Components -> Eletters and add some groups and if you want add some subscribers
* Install CronManager Now add the EletterQueue to CronManager.  If you are unable to set up CronManager 
    you could put the [[!EletterQueue]] Snippet in a document and call it manual, but that page will take 
    a couple of minutes to load!  

==========================================
 How to send your first newsletter 
==========================================
* Create a Resource/Document and select the EletterSample template.  Click on the Template Variables tab and fill out the information
...
* If you have CronManager set up then it will send out on the Publish Date or if it is published the next time it runs.
    If you don't have CronManager Set up put  [[!EletterQueue?]] in a page and load the page.  
...
Snippets -
    ...
    * EletterSignup
    * EletterUnsubscribe
    * EletterUpdateTables - you may need to run this after updating to latest version
