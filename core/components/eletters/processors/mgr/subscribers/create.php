    ...
    foreach($groups as $group) {
        ...
        if( isset($scriptProperties['groups_'.$id]) ) {
            ...
            if ( is_object($myGroup) ) {
                // this already exists
                $modx->log(modX::LOG_LEVEL_ERROR,'[ELetters/Process/create()] Subscriber Exists for group ('.$subscriber->get('id') .') to GroupID: '.$id);
                ...
                $modx->log(modX::LOG_LEVEL_ERROR,'[ELetters/Process/create()] add Subscriber for group ('.$subscriber->get('id') .') to GroupID: '.$id);
