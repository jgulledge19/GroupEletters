...
/* get obj */
if (empty($_DATA['id'])) return $modx->error->failure($modx->lexicon('eletters.groups.err.nf'));
$group = $modx->getObject('EletterGroups',$_DATA['id']);
if (empty($group)) return $modx->error->failure($modx->lexicon('eletters.groups.err.nf'));
...
/* save */
if ($group->save() == false) {
    return $modx->error->failure($modx->lexicon('eletters.groups.err.save'));
}
