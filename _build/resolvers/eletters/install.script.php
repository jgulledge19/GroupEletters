...
switch($options[xPDOTransport::PACKAGE_ACTION]) {
    ...
    case xPDOTransport::ACTION_INSTALL:
        ...
        if ($hasPlugins) {
            foreach($plugins as $name => $pluginEvents) {
                $pluginObj = $modx->getObject('modPlugin',array('name'=>$name));
                if (! $pluginObj) $modx->log(xPDO::LOG_LEVEL_INFO,'cannot get object: ' . $name);
                if (empty($pluginEvents)) $modx->log(xPDO::LOG_LEVEL_INFO,'Cannot get System Events');
                if (!empty ($pluginEvents) && $pluginObj) {

                    $modx->log(xPDO::LOG_LEVEL_INFO,'Assigning Events to Plugin ' . $name);
        ...
        if ($hasTemplates && $hasTemplateVariables) {
            ...
            if (!empty($templates)) {
                ...
                foreach ($templates as $templateName => $assignTvs) {
